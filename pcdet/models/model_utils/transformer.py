...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 ...
                 num_bins=8, max_num_nodes=20, use_one=False, pos_enc=False):
        super().__init__()
        ...
        if use_one =='encoder':
            ...
            decoder_norm = None
            self.decoder = None
            self.agg = MLP_v2([32,32])
        elif use_one =='decoder':
            encoder_layer = None
            encoder_norm = None
            self.encoder = None
    ...
    def _binning(self, x, num_bins, bin_idx):
        ...
        return x,occupied_mask

    def binning(self, x, idx, batch_dict):
        x = x[:,1:]
        idx, batch_idx = torch.sort(idx)
        x = x[batch_idx]
        x = t_b(x, idx, max_num_nodes=self.max_num_nodes)[0]
        x,occupied_mask = self._binning(x, self.num_bins, 0)
        return x,occupied_mask
    ...
    def forward(self, src, query_embed, unq_inv, data_dict):
        src, occupied_mask = self.binning(src, unq_inv, data_dict)
...
def build_transformer(args):
    return Transformer(
        ...
        use_one=args.get('use_one',False),
